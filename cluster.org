...
- The link above will also provide you with a list of commands to run if you get stuck
  ...
  #+BEGIN_SRC bash
  cd /home/ii/prow-config/prow/external-plugins/verify-conformance-request
  aws ecr get-login-password --region us-east-1 | docker login --username AWS --password-stdin 928655657136.dkr.ecr.us-east-1.amazonaws.com
  docker build -t cncf-infra .
  docker tag cncf-infra:latest 928655657136.dkr.ecr.us-east-1.amazonaws.com/cncf-infra:v1.4
  docker push 928655657136.dkr.ecr.us-east-1.amazonaws.com/cncf-infra:v1.4
  #+END_SRC
  ...
  Successfully tagged cncf-infra:latest
  The push refers to repository [928655657136.dkr.ecr.us-east-1.amazonaws.com/cncf-infra]
  ...
  59984c831a71: Layer already exists
  ...
  v1.4: digest: sha256:7374eaa38823120333d2ea8b31ceef0110e0dc6be34f775ae2054858ddee2da5 size: 2214
  #+end_src
...
* Next steps update verify-conformance-deployment.yaml to emulate the docker run
   - HH added a few notes to the Docker file, first suggestion is to not mount secrects in docker, but call the mount from k8-secrets
